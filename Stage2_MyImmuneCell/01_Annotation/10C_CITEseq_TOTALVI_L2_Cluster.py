...
def run_leiden_parallel(params):
    global adata
    key_added = f"L2_leiden_TOTALVI_{params['resolution']}"
    sc.tl.leiden(adata, key_added=f'L2_leiden_TOTALVI_{params['resolution']}', 
                 **params,
...
leiden_data = adata.obs.loc[:,['L2_leiden_TOTALVI_0.5', 'L2_leiden_TOTALVI_1', 'L2_leiden_TOTALVI_1.5', 'L2_leiden_TOTALVI_2','L2_leiden_TOTALVI_3']]
obsm = adata.obsm
...
sc.pl.umap(
    ...
    save=f'_{dataset}_TOTALVI_L2_annoy_leiden',show=False
)
...
sc.pl.umap(
    adata,
    color=['Reference_Atlas_L1L2_mv', 'AIFI_L2','Immune_All_Low','Adult_Human_Blood','predicted.celltype.l2','scDblFinder.class'],
    legend_fontsize=6,legend_loc='on data',ncols=3,frameon=False,
...
sc.pl.umap(
    ...
    layer='clr',
    save=f'_{dataset}_scADT_TOTALVI_L2_annoy_leiden'
    )
