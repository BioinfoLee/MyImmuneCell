...
GWAS_dir = '/media/AnalysisDisk2/Yangshichen/2_My-Onek/WGS/Data/GWAS_ma/ma_file_for_SMR_TPMI/'
...
trait_all = pd.read_csv('/media/AnalysisDisk2/Yangshichen/2_My-Onek/WGS/Data/GWAS_ma/TPMI_disease.txt',sep='\t',header=None,dtype=str)
trait_all = trait_all[0].to_list()
trait_all = trait_all[13:]
...
for trait in trait_all:
    ...
    os.system(f"""awk 'NR==1{{print $0}} NR>1{{split($1,a,\"_\");chrom=a[1];sub(/^chr/,"",chrom); print chrom\":\"a[2]\":\"$3\":\"$2, $2, $3, $4, $5, $6, $7, $8}}' {gwas_file} > {gwas_tmp}""") 
    
    from concurrent.futures import ThreadPoolExecutor, as_completed
    def run_smr(celltype):
        eQTL = f"{eQTL_output_dir}{celltype}"
        gi = f'"{smr}" --bfile "{bim}" --gwas-summary {gwas_tmp} --beqtl-summary "{eQTL}" --thread-num 10 --out "{trait_dir}/eQTLsmr_{celltype}"'
        return os.system(gi)
